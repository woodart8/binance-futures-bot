...
- **롱**: 가격이 박스 **하단 근처**(4% 이내). (MA 조건 없음 — 반등 반영 전 저점에서 진입 가능)
- **숏**: 가격이 박스 **상단 근처**(4% 이내). (MA 조건 없음)
- 청산: 목표 익절(SIDEWAYS_PROFIT_TARGET), 손절(SIDEWAYS_STOP_LOSS), 박스 이탈(상·하단 가격 0.5% 돌파 시, 가격 기준), 전략 신호(추세 전환).
- **장세 전환**: 가격이 박스권을 0.5% 돌파하면 **추세장**으로 간주하며, 가격이 다시 박스 안으로 들어올 때까지 횡보장 진입 없음.
