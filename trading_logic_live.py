...
def process_live_candle(exchange, state: Dict[str, Any], df: pd.DataFrame) -> Tuple[Dict[str, Any], bool]:
    """
    새 캔들 시점에서 신호/청산/진입 처리. exchange 주문 호출 포함.
    반환: (업데이트된 state, skip)
    skip=True 이면 호출자가 last_candle_time 갱신 없이 다음 루프로 continue (재시도).
    """
    latest = df.iloc[-1]
