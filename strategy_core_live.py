...
def _find_recent_pivots(
    ...
    *,
    ...
    pivots: List[int] = []
    # 뒤에서부터 스캔하면서 중심값이 양옆 left_right 봉보다 크거나/작으면 피벗
...
def _compute_pivot_divergences(
    ...
    if len(lows_idx) >= 2:
        ...
        # 일반 상승 다이버전스: 저점2 가격 < 저점1 가격, 저점2 RSI > 저점1 RSI
        regular_bull = p2 < p1 and r2 > r1
        # 히든 상승 다이버전스: 저점2 가격 > 저점1 가격, 저점2 RSI < 저점1 RSI
    ...
    if len(highs_idx) >= 2:
        ...
        p1, p2 = closes[i1], closes[i2]
        r1, r2 = rsis[i1], rsis[i2]
        # 일반 하락 다이버전스: 고점2 가격 > 고점1 가격, 고점2 RSI < 고점1 RSI
        regular_bear = p2 > p1 and r2 < r1
        # 히든 하락 다이버전스: 고점2 가격 < 고점1 가격, 고점2 RSI > 고점1 RSI
